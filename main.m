function [feature_matrix,confusion_matrix]= main(filepath,model_name)
    ...
    list1={};
    list2={};
    all_data_No_Label_Trigger_event=[];
...
for k=1:10
    ...
    %% Creating a VEC for the 3 axis of ACC & GYRO
    for i=1:12
    ...
    %% Filtering the signal using IMF
        ...
        for segment=1:length(time)-1
            ...
            if max(max(ind))<= length(acc_f_allsig{i})
                ...
                if event_trigger(acc_f_allsig{i},ind) & event_trigger(gyro_f_allsig{i},ind) == 1; % Trigger Condtion for window
                    ...
                    x=extract_features_acc_triger_submit(acc_f_allsig{i},5,25,ind);
        ...
        for segment=1:length(time)-1
            ...
            if max(max(ind))<= length(gyro_f_allsig{i})
                ...
                if event_trigger(acc_f_allsig{i},ind) & event_trigger(gyro_f_allsig{i},ind) == 1; % Trigger Condtion for window
                    ...
                    g=extract_features_Trigger_Gyro_submit(gyro_f_allsig{i},5,25,ind);
    ...
    %%
    data_Trigger_event              =[acc_Filterd_matrix gyro_Filterd_matrix final_label];
    data_matrix_Trigger_event             =[acc_Filterd_matrix gyro_Filterd_matrix]; 
    %the Big matrix of acc+gyro windows + features 
    all_data_No_Label_Trigger_event =[all_data_No_Label_Trigger_event ; data_matrix_Trigger_event];
    labels_Trigger_event                  =[labels_Trigger_event ; final_label];
    k

    %end
    %sorting the data
    final_data=[all_data_No_Label_Trigger_event(:,2) all_data_No_Label_Trigger_event(:,3) all_data_No_Label_Trigger_event(:,8) all_data_No_Label_Trigger_event(:,9) all_data_No_Label_Trigger_event(:,6) all_data_No_Label_Trigger_event(:,4) all_data_No_Label_Trigger_event(:,7) all_data_No_Label_Trigger_event(:,10) all_data_No_Label_Trigger_event(:,1) all_data_No_Label_Trigger_event(:,5)];
    catch
...
tab=tabulate(labels_Trigger_event);
tic;
[Yfit_bag,score]    = predict(bagTree_submit,final_data); % Prediction
disp('Prediction time for RandomForest prediction:');
toc
disp('Confusion matrix (percentage) with Random Forest:');
confusion_matrix =confusionmat(labels_Trigger_event,Yfit_bag);  % confuiso matrix
figure;
confusionchart(labels_Trigger_event,Yfit_bag,'RowSummary','absolute','ColumnSummary','absolute');
feature_matrix=final_data;
