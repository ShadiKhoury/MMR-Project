%% Feature Selection
function feat_vec=extract_features_Gyro_window(f_sig,n_features,sample_rate,ind)
...
feat_vec(4)  = length(findpeaks(new_sig(:,1)));                    %feature number of peaks in x  
feat_vec(5)  = length(findpeaks(new_sig(:,2)));                    %feature number of peaks in y  
feat_vec(6)  = length(findpeaks(new_sig(:,3)));                    %feature number of peaks in z  
feat_vec(7)  = sum(isoutlier(f_sig(:,1)));                         % isouliner 5
feat_vec(8)  = kurtosis(new_sig(:,1));                             % Kurtosis 6
...
feat_vec(9)  = sum(Px);  feat_vec(10) = sum(Py); feat_vec(11) = sum(Pz); 
...
feat_vec(12) = f_max_mag_x(1);   %the Frequancy of the maximan Power of the feature x 
feat_vec(13) = f_max_mag_y(1);   %the Frequancy of the maximan Power of the feature y 
feat_vec(14) = f_max_mag_z(1);   %the Frequancy of the maximan Power of the feature z 
...
EE                 =   sqrt(new_sig.^2);
feat_vec(19)       =   max(max(EE));

feat_vec(20)       =  entropy(normalize((x_sig)));
feat_vec(21)       =  entropy(normalize((y_sig)));
feat_vec(22)       =  entropy(normalize((z_sig)));
feat_vec(23)       =  rms(z_sig)^2-rms(y_sig)^2;
%feat_vec(24)       =  rms(x_sig)^2-rms(z_sig)^2;
%%
end
