%% Feature Selection
function feat_vec=extract_features_ACC_window(f_sig,n_features,sample_rate,ind)
...
feat_vec(7) = f_max_mag_x(1);   %the Frequancy of the maximan Power of the feature x 
feat_vec(8) = f_max_mag_y(1);   %the Frequancy of the maximan Power of the feature y 
feat_vec(9) = f_max_mag_z(1);   %the Frequancy of the maximan Power of the feature z 
...
feat_vec(10)       = (std(first_half)-std(second_half));              %comparing std between windows parts 4
%% FFT and Amplitued 
fft_X              = fft(new_sig(:,1));% X axis
amp_X              = 20*log(abs(real(fft_X)));
feat_vec(11)       = std(amp_X);
fft_Y              = fft(new_sig(:,2));% Y axis
amp_Y              = 20*log(abs(real(fft_Y)));
feat_vec(12)       = std(amp_Y);
...
EE                 =   sqrt(new_sig.^2);
feat_vec(14)       =   max(max(EE));
%%
feat_vec(15)       =  entropy(normalize((x_sig)));
feat_vec(16)       =  entropy(normalize((y_sig)));
feat_vec(17)       =  entropy(normalize((z_sig)));
%feat_vec(19)       =  rms(z_sig)^2-rms(y_sig)^2;
